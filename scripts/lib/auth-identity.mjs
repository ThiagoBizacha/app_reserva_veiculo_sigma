...
export function buildUsernameSeedFromFullName(fullName) {
  ...
  const lastToken = [...tokens].reverse().find((token) => !USERNAME_CONNECTORS.has(token)) ?? tokens[tokens.length - 1];

  return normalizeUsernameInput(`${firstToken.slice(0, 1)}${lastToken}`) || "USER";
}

export function buildUniqueUsername(fullName, takenUsernames, preferredUsername = "") {
  const normalizedTaken = new Set(
    Array.from(takenUsernames, (value) => normalizeUsernameInput(value)).filter(Boolean)
  );
  const baseUsername = normalizeUsernameInput(preferredUsername) || buildUsernameSeedFromFullName(fullName);
...
export function buildAuthLoginEmail(email, username) {
